...
class line_tracing():
    ...
    def lidar_callback(self, _data):
        cnt = 0 # obstacle particle
        cnt_l = 0 # check car on the left
        cnt_r = 0 # check car on the right
        ...
        for i, angle in enumerate(angle_deg):
            if -15 <= angle <= 15 and 0.0 < _data.ranges[i] < 1.0: # depends on car's width
                cnt += 1
            if 49 <= angle <= 79 and 0.0 < _data.ranges[i] < 1.0:
                cnt_l += 1
            if -79 <= angle <= -49 and 0.0 < _data.ranges[i] < 1.0:
                cnt_r += 1
            if cnt >= 5:
                if (cnt_l >= 5)&(cnt_r <= 5):
                    self.mode = 2 # move right
                elif (cnt_r >= 5)&(cnt_l <= 5):
                    self.mode = 3 # move left
                elif (cnt_l >= 5)&(cnt_r >= 5):
                    self.mode = 0 # stop
                elif (cnt_l <= 5)&(cnt_r <= 5):
                    self.mode = 2 # move right
            else:
                self.mode = 1 # move forward
    ...
    def callback(self, _data):
        ...
        elif self.mode == 2: # obstacle on the left
            self.moveright()
        elif self.mode == 3: # obstacle on the right
            self.moveleft()
